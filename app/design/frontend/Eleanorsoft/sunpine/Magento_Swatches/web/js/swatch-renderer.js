...
define([
    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _RenderControls: function () {
            ...
            $('.selectric-color').selectric({
                optionsItemBuilder: function(itemData, element, index) {
                    return itemData.element.data('value') ? itemData.text + '<span class="color-swatch" style="background-color:' + itemData.element.data('value') + ';display:inline-block;width:20px;height:20px;border-radius:50%;vertical-align:middle;margin-left:5px;"></span>' : itemData.text;
                },
                inheritOriginalWidth: true
            });
            $('.selectric-color').change(function () {
                var id = $(this).val();
                if (id) {
                    $('div[option-id="' + id + '"]:not(.selected)').click();
                }
            });
        ...
        _RenderSwatchSelect: function (config, chooseText) {
            ...
            if (this.options.jsonSwatchConfig.hasOwnProperty(config.id)) {
                var optionConfig = this.options.jsonSwatchConfig[config.id];
                html =
                    '<select class="' + this.options.classes.selectClass + ' ' + config.code + ' selectric-color">' +
                    '<option value="0" option-id="0">' + config.label + '</option>';
                $.each(config.options, function () {
                    var id = this.id;
                    if (optionConfig.hasOwnProperty(id)) {
                        var value = optionConfig[id].hasOwnProperty('value') ? optionConfig[id].value : '';
                ...
                html =
                    '<select class="' + this.options.classes.selectClass + ' ' + config.code + ' selectric-select">' +
                    '<option value="0" option-id="0">' + config.label + '</option>';
