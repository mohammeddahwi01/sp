...
/**
 * This file was modified to include invisible recaptcha in the form.
 * @author Konstantin Esin <hello@eleanorsoft.com>
 */
 
/**
 * Callback for recaptcha validation process.
 * See documentation on invisible recaptcha for details.
 */
function invisibleRecaptchaCallback(token)
{
	invisibleRecaptchaCallbackFunction(token);
}
var invisibleRecaptchaCallbackFunction = function () {}; // this function will be set in ajaxCommentActions() 
...
require([
    ...
    function ajaxCommentActions(cmtText, inputEl, checkReply, cmtId, parentComment) {
		submitCmt.attr('disabled', true); // disable submit button
		...
		invisibleRecaptchaCallbackFunction = function (token) {
			$.ajax({
				type: 'POST',
				url: window.location.href,
				data: {cmt_text: cmtText, isReply: isReply, replyId: replyId, 'g-recaptcha-response': token, 'amasty_invisible_token': token}, // added recaptcha tokens. "amasty_invisible_token" is required for module Invisible ReCaptcha by Amasty
				success: function (response) {
					if (response.status === 'ok') {
				...
				complete: function () {
					submitCmt.attr('disabled', false); // enable submit button
				}
			});
		};
		grecaptcha.execute(); // run recaptcha validation
