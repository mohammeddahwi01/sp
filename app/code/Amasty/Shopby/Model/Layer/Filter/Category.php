...
class Category extends \Magento\Catalog\Model\Layer\Filter\AbstractFilter
{
    ...
    public function apply(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if($setting->isMultiselect() && $request->getParam('id') != $categoryId){
            ...
            $productCollection->addFieldToFilter('category_ids', $categoryIds);
    ...
    protected function getExtendedCategoryData()
    {
        ...
        $collection->addAttributeToSelect(
            ...
            'is_active',
            1
        )->addFieldToFilter('path', ['like'=>$startPath.'%'])
            ->addFieldToFilter('level', ['gt'=>$minLevel])
            ->addFieldToFilter('level', ['lteq'=>$maxLevel])
            ->setOrder(
                'position',
                \Magento\Framework\DB\Select::SQL_ASC
            );
        $collection->getSelect()->joinLeft(['parent'=>$collection->getMainTable()],'e.parent_id = parent.entity_id', ['parent_path'=>'parent.path']);
    ...
    protected function getFacetedData()
    {
        ...
        if(
            ...
            $requestBuilder->bind('category_ids', $startCategoryForCountBucket->getId());
