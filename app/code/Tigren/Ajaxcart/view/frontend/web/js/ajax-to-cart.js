define([
    ...
    $.widget('tigren.ajaxToCart', $.tigren.ajaxSuite, {
        ...
        initEvents: function() {
            var self = this;
            $(self.options.ajaxCart.addToCartButtonSelector).on('click', function(e) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.html_popup) {
                        ...
                        self.options.cartWrapper.find('.product-add-form .product-options-wrapper').css({'float':'none', 'width':'100%'});
                        self.options.cartWrapper.find('.product-add-form .product-options-wrapper .product-options-bottom').css({'float':'none', 'width':'100%'});
        ...
        ajaxPopupSubmit: function(form) { 
            ...
            if (inputItemWishlist.length && inputItemWishlist.val()){
                isWishlist = true;
                if (actionUrl.search('checkout/cart/add') != -1) {
                    additionUrl = actionUrl.replace(self.options.ajaxCart.addToCartUrl, "");
                }
            }
                ...
                if (afterloadElm == 'swatch') {
                    if (self.options.cartWrapper.find('.swatch-opt').length > 0) {
                        var cartCenterInterval = setInterval(function() {
                            if (self.options.cartWrapper.find('.swatch-attribute.color').length > 0 && self.options.cartWrapper.find('.swatch-attribute.size').length > 0) {
