...
class RevSliderSlide extends \Nwdthemes\Revslider\Model\Revslider\Framework\RevSliderElementsBase {
	...
	public function initByData($record){
        $record = $this->_framework->apply_filters('revslider_slide_initByData', $record);
	...
	public function setImageByID($imageID, $size = 'full'){
        $a = $this->_framework->apply_filters('revslider_slide_setImageByID', array('imageID' => $imageID, 'size' => $size), $this);
	...
	public function setBackgroundType($new_param){
        $new_param = $this->_framework->apply_filters('revslider_slide_setBackgroundType', $new_param, $this);
	...
	public function initBySlide(RevSliderSlide $slide){
        $slide = $this->_framework->apply_filters('revslider_slide_initBySlide', $slide, $this);
	...
	public function initByStreamData($postData, $slideTemplate, $sliderID, $sourceType, $additions){
        $a = $this->_framework->apply_filters('revslider_slide_initByStreamData', array('postData' => $postData, 'slideTemplate' => $slideTemplate, 'sliderID' => $sliderID, 'sourceType' => $sourceType, 'additions' => $additions), $this);
        ...
        switch($a['sourceType']){
			...
			break;
			default:
                $return = $this->_framework->apply_filters('revslider_slide_initByStreamData_sourceType', false, $a, $this);
                if($return === false)
	...
	private function initByFacebook($sliderID, $additions){
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByFacebook_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fb.png';
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByFacebook_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByTwitter($sliderID, $additions){
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByTwitter_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamtwitter' || $this->params["background_type"] == 'streamtwitterboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/tw.png';
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByTwitter_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByFlickr($sliderID){
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByFlickr_pre', $this->postData, $sliderID, $this);
		...
		if($this->params["background_type"] == 'image'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fr.png';
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByFlickr_post', $this->postData, $sliderID, $this);
		//replace placeholders in layers:
	...
	private function initByYoutube($sliderID, $additions){
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByYoutube_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamyoutube' || $this->params["background_type"] == 'streamyoutubeboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/yt.png';
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByYoutube_post', $this->postData, $sliderID, $additions, $this);
	...
	private function initByVimeo($sliderID, $additions){
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByVimeo_pre', $this->postData, $sliderID, $additions, $this);
		...
		if($this->params["background_type"] == 'trans' || $this->params["background_type"] == 'image' || $this->params["background_type"] == 'streamvimeo' || $this->params["background_type"] == 'streamvimeoboth'){ //if image is choosen, use featured image as background
			...
			if(empty($this->imageUrl))
				$this->imageUrl = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vm.png';
        ...
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByVimeo_post', $this->postData, $sliderID, $additions, $this);
	...
	private function setLayersByStreamData($sliderID, $stream_type, $additions = array()){
        ...
        $a = $this->_framework->apply_filters('revslider_slide_setLayersByStreamData_pre', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
		...
		foreach($this->arrLayers as $key=>$layer){
			...
			$text = $this->_framework->apply_filters('revslider_mod_stream_meta', $text, $sliderID, $stream_type, $this->postData); //option to add your own filter here to modify meta to your likings
        ...
        $a = $this->_framework->apply_filters('revslider_slide_setLayersByStreamData_post', array('arrLayers' => $this->arrLayers, 'params' => $this->params), $sliderID, $stream_type, $additions, $this);
	...
	public function set_stream_data($text, $attr, $stream_type, $additions = array(), $is_action = false){
        ...
        $text = $this->_framework->apply_filters('revslider_slide_set_stream_data_pre', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
        ...
        return $this->_framework->apply_filters('revslider_slide_set_stream_data_post', $text, $attr, $stream_type, $additions, $is_action, $img_sizes);
	}
	...
	public function return_stream_data($stream_type, $additions = array()){
        ...
        $attr = $this->_framework->apply_filters('revslider_slide_return_stream_data_pre', $attr, $stream_type, $additions, $img_sizes);
        ...
        return $this->_framework->apply_filters('revslider_slide_return_stream_data_post', $attr, $stream_type, $additions, $img_sizes);
	}
	...
	public function get_facebook_timeline_image(){
        ...
        return $this->_framework->apply_filters('revslider_slide_get_facebook_timeline_image', $return, $object_id, $picture, $this);
	}
	...
	public function initByPostData($postData, RevSliderSlide $slideTemplate, $sliderID){
        $this->postData = $this->_framework->apply_filters('revslider_slide_initByPostData', $postData, $slideTemplate, $sliderID, $this);

		$postID = $postData["ID"];

		$slideTemplateID = $this->_framework->get_post_meta($postID, 'slide_template', true);
		...
		if(!empty($slideTemplateID) && is_numeric($slideTemplateID)){
			...
			try{
				...
				$slideTemplateLocal = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
		...
		if(isset($this->params['enable_link']) && $this->params['enable_link'] == "true" && isset($this->params['link_type']) && $this->params['link_type'] == "regular"){
			$link = $this->_framework->get_permalink($postID);
			$this->params["link"] = str_replace(array("%link%", '{{link}}'), $link, $this->params["link"]);
			...
			foreach($arrMatches as $match){
				...
				$meta = str_replace("%","",$meta);
				$meta = str_replace('_REVSLIDER_', '-', $meta);
				$metaValue = $this->_framework->get_post_meta($postID,$meta,true);
	...
	private function setLayersByPostData($postData,$sliderID){
        $postData = $this->_framework->apply_filters('revslider_slide_setLayersByPostData_pre', $postData, $sliderID, $this);
        ...
        if($st == 'woocommerce'){
            $excerpt_limit = $this->getSliderParam($sliderID,"excerpt_limit_product",55,RevSliderSlider::VALIDATE_NUMERIC);
            $excerpt_limit = (int)$excerpt_limit;
        }else{
		$excerpt_limit = $this->getSliderParam($sliderID,"excerpt_limit",55,RevSliderSlider::VALIDATE_NUMERIC);
		$excerpt_limit = (int)$excerpt_limit;
        }
		...
		$attr['link'] = $this->_framework->get_permalink($postID);
		...
		$ptid = $this->_framework->get_post_thumbnail_id($postID);
		
		$img_sizes = RevSliderBase::get_all_image_sizes();
		$attr['img_urls'] = array();
		foreach($img_sizes as $img_handle => $img_name){
			$featured_image_url = $this->_images->wp_get_attachment_image_src($ptid, $img_handle);
        ...
        $attr = $this->_framework->apply_filters('revslider_slide_setLayersByPostData_post', $attr, $postData, $sliderID, $this);
        
		foreach($this->arrLayers as $key=>$layer){
			
			$text = RevSliderFunctions::getVal($layer, "text");
			$text = $this->_framework->apply_filters('revslider_mod_meta', $text, $postID, $postData); //option to add your own filter here to modify meta to your likings
	...
	public function set_post_data($text, $attr, $post_id){
        ...
        $text = $this->_framework->apply_filters( 'rev_slider_insert_meta', $text, $post_id );
		...
		foreach($arrMatches as $matched){
			...
			foreach($matched as $match) {
				...
				$meta = str_replace("%","",$meta);
				$meta = str_replace('_REVSLIDER_', '-', $meta);
				$metaValue = $this->_framework->get_post_meta($post_id,$meta,true);
    ...
    public function getDataByID($slideid){
        ...
        return $this->_framework->apply_filters('revslider_slide_getDataByID', $return, $slideid, $this);
    }
	...
	private function setImageByImageID($imageID){
        ...
        $imageID = $this->_framework->apply_filters('revslider_slide_setImageByImageID', $imageID, $this);
	...
	public function getArrChildren(){
        ...
        return $this->_framework->apply_filters('revslider_slide_getArrChildren', $this->arrChildren, $this);
	}
	...
	public function getParentSlide(){
		...
		$parentSlide = new RevSliderSlide($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
	...
	public function getLayers(){
		$this->validateInited();
        return $this->_framework->apply_filters('revslider_getLayers', $this->arrLayers, $this);
	}
	...
	public function saveLayers(){
        ...
        $this->arrLayers = $this->_framework->apply_filters('revslider_slide_saveLayers', $this->arrLayers, $this->static_slide, $this);
	...
	public function saveParams(){
        ...
        $this->params = $this->_framework->apply_filters('revslider_slide_saveParams', $this->params, $this->static_slide, $this);
	...
	public function getLayersForExport($useDummy = false){
        ...
        return $this->_framework->apply_filters('revslider_slide_getLayersForExport', $arrLayersNew, $this);
	}
	...
	public function getParamsForExport(){
        ...
        return $this->_framework->apply_filters('revslider_slide_getParamsForExport', $arrParams, $this);
	}
	...
	public function getLayersNormalizeText(){
        ...
        return $this->_framework->apply_filters('revslider_slide_getLayersNormalizeText', $arrLayersNew, $this);
	}
	...
	public function getParams(){
		$this->validateInited();
        return $this->_framework->apply_filters('revslider_slide_getParams', $this->params, $this);
	}
	...
	public function getSettings(){
		$this->validateInited();
        return $this->_framework->apply_filters('revslider_slide_getSettings', $this->settings, $this);
	}
	...
	public function createSlide($sliderID,$obj="",$static = false){
        ...
        $arrInsert = $this->_framework->apply_filters('revslider_slide_createSlide', $arrInsert, $sliderID, $static, $this);
    ...
    protected function updateParamsInDB($arrUpdate = array(), $static = false){
        ...
        $this->params = $this->_framework->apply_filters('revslider_slide_updateParamsInDB', array_merge($this->params,$arrUpdate), $this);
	...
	protected function updateLayersInDB($arrLayers = null, $static = false){
        ...
        $arrLayers = $this->_framework->apply_filters('revslider_slide_updateLayersInDB', $arrLayers, $this);
	...
	public function updateSlideFromData($data){
        ...
        $arrUpdate = $this->_framework->apply_filters('revslider_slide_updateSlideFromData_pre', $arrUpdate, $data, $this);

		$this->db->update(RevSliderGlobals::$table_slides,$arrUpdate,array("id"=>$this->id));
		
        $this->_framework->do_action('revslider_slide_updateSlideFromData_post', $arrUpdate, $data, $this);
	}
	...
	public function updateStaticSlideFromData($data){
        ...
        $arrUpdate = $this->_framework->apply_filters('revslider_slide_updateStaticSlideFromData', $arrUpdate, $data, $this);
        
		$this->db->update(RevSliderGlobals::$table_static_slides,$arrUpdate,array("id"=>$this->id));
		
        $this->_framework->do_action('revslider_slide_updateStaticSlideFromData_post', $arrUpdate, $data, $this);
	...
	public function deleteSlide(){
        ...
        $this->_framework->do_action('revslider_slide_deleteSlide', $this->id);
	}
	...
	public function updateTitleByID($data){
        ...
        $arrUpdate = $this->_framework->apply_filters('revslider_slide_updateTitleByID', $arrUpdate, $data, $this);
	...
	public function toggleSlideStatFromData($data){
        ...
        $arrUpdate = $this->_framework->apply_filters('revslider_slide_toggleSlideStatFromData', $arrUpdate, $data, $this);
	...
	public function getUrlImageThumb(){
        ...
        $urlImage = $this->_framework->apply_filters('revslider_slide_getUrlImageThumb', $urlImage, $this);
	...
	public function get_image_attributes($slider_type){
		...
		switch($slider_type){
			...
			break;
			case 'posts':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/post.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'woocommerce':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/wc.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'facebook':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fb.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'twitter':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/tw.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'instagram':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/ig.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'flickr':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/fr.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'youtube':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/yt.png';
				$bgStyle = 'background-size: cover;';
			break;
			case 'vimeo':
				$thumb = Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vm.png';
				$bgStyle = 'background-size: cover;';
			break;
		}
        ...
        return $this->_framework->apply_filters('revslider_slide_get_image_attributes', array(
			'url' => $this->_images->imageUrl($data_urlImageForView),
			'class' => $bg_extraClass,
			'style' => $bg_fullstyle
        ), $this);
	...
	public function replaceImageUrls($urlFrom, $urlTo, $static = false){
        ...
        $this->_framework->do_action('revslider_slide_replaceImageUrls', $this);
	}
	...
	public function getUsedFonts($full = false){
		...
		$op = new RevSliderOperations($this->_framework, $this->_query, $this->_curl, $this->_filesystem, $this->_images, $this->_resource, $this->_googleFonts, $this->_registerHelper);
		$fonts = array();
        ...
        return $this->_framework->apply_filters('revslider_slide_getUsedFonts', $fonts, $this);
	}
	...
	public function replaceCssClass($css_from, $css_to){
        ...
        $this->_framework->do_action('revslider_slide_replaceCssClass', $css_from, $css_to, $this);
	}
	...
	public function reset_slide_values($values){
		...
		foreach($values as $key => $val){
			if($key !== 'sliderid'){
				$this->params[$this->_framework->esc_attr($key)] = $this->_framework->esc_attr($val);
			}
		}
	...
	public static function translateIntoSizes(){
        ...
        return self::$framework->apply_filters('revslider_slide_translateIntoSizes', array(
			'align_hor',
			'align_vert',
			'top',
			'left',
	...
	private function updateLayersImages($layers = array()) {
		foreach ($layers as $key => $layer) {
			if (isset($layer['image_url']) && ! empty($layer['image_url'])) {
				$layers[$key]['image_url'] = $this->_images->imageUrl($layer['image_url']);
			}
			if (isset($layer['video_data']) && isset($layer['video_data']->urlAudio) && ! empty($layer['video_data']->urlAudio)) {
				$layers[$key]['video_data']->urlAudio = $this->_images->imageUrl($layer['video_data']->urlAudio);
			}
		}
		return $layers;
	}
